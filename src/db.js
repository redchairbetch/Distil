...
function assemblePatient(row) {
  ...
  return {
    ...
    audiology: {
      ...
      // Presentation level each speech score was obtained at. Stored so the
      // number is portable across clinics; NULL means "not recorded", never 45.
      unaidedLevelR: audiogram?.unaided_wrs_level_right ?? null,
      unaidedLevelL: audiogram?.unaided_wrs_level_left  ?? null,
      // Binaural unaided WRS — administered when monaural discrimination is poor.
      unaidedBin:      audiogram?.unaided_wrs_binaural       ?? null,
      unaidedLevelBin: audiogram?.unaided_wrs_level_binaural ?? null,
      ...
      wrMclLevelR: audiogram?.wr_mcl_level_right ?? null,
      wrMclLevelL: audiogram?.wr_mcl_level_left  ?? null,
      sinBin:   audiogram?.sin_score         ?? null,
      // QuickSIN / WIN / BKB-SIN do not share a scale — carry the test identity,
      // ear, and presentation level so sin_score is interpretable.
      sinTest:  audiogram?.sin_test          ?? null,
      sinEar:   audiogram?.sin_ear           ?? null,
      sinLevel: audiogram?.sin_level         ?? null,
    ...
    if (hasAudioData) {
      const { data: audioRow, error: audioError } = await supabase
        ...
        .insert({
          ...
          // Protocol default (45 / quicksin / binaural) applied only when the
          // score itself exists — a missing measurement never gets a condition.
          unaided_wrs_level_right: a.unaidedLevelR ?? (a.unaidedR   != null ? 45 : null),
          unaided_wrs_level_left:  a.unaidedLevelL ?? (a.unaidedL   != null ? 45 : null),
          unaided_wrs_binaural:       a.unaidedBin      ?? null,
          unaided_wrs_level_binaural: a.unaidedLevelBin ?? (a.unaidedBin != null ? 45 : null),
          aided_wrs_right:   a.aidedR   ?? null,
          aided_wrs_left:    a.aidedL   ?? null,
          wr_mcl_right:      a.wrMclR   ?? null,
          wr_mcl_left:       a.wrMclL   ?? null,
          wr_mcl_level_right: a.wrMclLevelR ?? null,
          wr_mcl_level_left:  a.wrMclLevelL ?? null,
          sin_score:         a.sinBin   ?? null,
          sin_test:          a.sinTest  ?? (a.sinBin != null ? 'quicksin' : null),
          sin_ear:           a.sinEar   ?? (a.sinBin != null ? 'binaural' : null),
          sin_level:         a.sinLevel ?? null,
