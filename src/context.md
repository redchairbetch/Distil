...
23. **Upgrade Patient Journey (established-patient flow)** — Today the only path through the wizard is the new-patient flow: every field from name to insurance to audiogram entered from scratch. Established patients returning for a follow-up visit or upgrade conversation get force-marched through the same 8-step form even though most of the patient-info fields haven't changed. Need a dedicated established-patient flow that:
    ...
    - **Kiosk side — Phase 2 (deferred)**: cross-device prefill so a returning patient *reviews last year's answers* on the kiosk. Needs a provider-initiated handoff (single-use token row + anon-callable edge function returning the prior contact + readiness payload, same `verify_jwt:false` self-auth pattern as `subscribe-push`/`aided-patient-feed`) + a CRM "Launch upgrade check-in" button that mints the token. The `?visit=upgrade` param is the intended token carrier. PHI must not go in the URL — only the opaque token.
