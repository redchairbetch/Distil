...
13. **Private-pay quote: bundled care plan + tier-aware pricing** — three coordinated changes to `generateQuote.js` (and likely `generatePurchaseAgreement.js`) when `payType === "private"`: (a) the "RECOMMENDED CARE PLAN" section header reads "INCLUDED CARE PLAN" because Complete Care+ is bundled into the device price for private pay, and totals shouldn't double-count care plan cost; (b) Complete Care+ still renders as a transparent zero-charge line item ("Complete Care+ — Included, no charge") so the patient sees what's covered without thinking they're being upcharged; (c) per-aid pricing switches from the hardcoded `privatePayPrimaryPerAid()` ($2,750 Signia / $3,250 other) to the differentiated `clinic_retail_anchors.price_per_aid` for the patient's chosen tier so the quote actually matches the tier cards in the MSRP editor.
