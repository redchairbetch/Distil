...
12. **Audit `clinic_retail_anchors` 'standard' rows** — DB currently has 5 rows with `manufacturer_class='standard'` alongside 5 `'signia'` rows. Surfaced in Settings → Retail Anchors as "Standard (general retail)". Determine if they're live (cross-reference `insurance_plans.retail_anchor_key`) and decide: keep as the manufacturer-agnostic baseline, fold into `'signia'`, or delete. Low-priority data hygiene.
13. **Private-pay quote: bundled care plan + tier-aware pricing** — Phase 1 ✅ DONE: `generateQuote.js` and `generatePurchaseAgreement.js` now switch the care plan section to "INCLUDED CARE PLAN" when `payType === "private"`, render Complete Care+ as a zero-charge line item ("Bundled with your device purchase — no separate charge"), and stop double-counting the care plan in totals. The wizard quote/PA handlers feed `form.tierPrice` (set by `TierSelection` from `clinic_retail_anchors`) instead of the hardcoded `privatePayPrimaryPerAid()` $2,750/$3,250 split. **Phase 2 (pending)**: persist the chosen private-pay tier + per-aid price on the patient record so re-generated quotes/PAs from the patient list aren't stuck on the legacy $2,750 fallback. Currently `finalizePatient` only writes `insurance_coverage.tier`/`tier_price_per_aid` when `payType === "insurance"`. Likely path: add `private_pay_tier` (text) and `private_pay_price_per_aid` (numeric) to `patients`, wire through `finalizePatient` / `loadAllPatients` / `loadPatient`, update the patient-list quote handler (Distil.jsx ~5556) and PA handler (~5620) to read them, and remove the two `// TODO Phase 2` comments left in place.
