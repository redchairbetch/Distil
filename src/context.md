...
13. **Private-pay quote: bundled care plan + tier-aware pricing** — two corrections to `generateQuote.js` (and likely `generatePurchaseAgreement.js`) when `payType === "private"`: (a) the "RECOMMENDED CARE PLAN" section header should read "INCLUDED CARE PLAN" because Complete Care+ is bundled into the device price for private pay, and the totals shouldn't double-count care plan cost; (b) per-aid pricing should switch from the hardcoded `privatePayPrimaryPerAid()` ($2,750 Signia / $3,250 other) to the differentiated `clinic_retail_anchors.price_per_aid` so the quote actually reflects the tier the patient picked. Source-of-truth question still open with Kurt — confirm whether to read from `clinic_retail_anchors` directly or via the MSRP editor's effective price.
