...
21. **Defer tier price display until full device selection** — Field-tested on a new retail patient (2026-05-12): selecting `Premium` on TierSelection causes the next screen to display `$4,497.50` per aid before any manufacturer/family/style is picked. Two issues bundled: (a) the price shows prematurely — should remain blank or `$0` until the patient has selected manufacturer + family + tech level + style, then resolve and reveal once the full configuration is locked; (b) the price displayed when Premium is the chosen tier is `$4,497.50` (rank 4, Advanced) rather than the standard-class rank-5 baseline of `$4,997.50`, suggesting a tier→rank mapping bug on the standard ladder. Signia 7IX correctly resolves to `$3,997.50` once a Signia device is selected, so the per-aid pricing path works end-to-end — the bug is in the pre-device gating + the Premium-as-rank-4 mismap. Related to #18 (manufacturer- and tech-level-aware pricing) and #19 (per-ear/CROS pricing) but separate concern: this is about *when* the price appears and *what baseline* shows during the gap.
22. **Remove `ChapterIntro` overlay pop-ups from Narrative Thread** — The chapter-intro overlay shipped in Narrative Thread PR 1 (`src/components/ChapterIntro.jsx`, fires between wizard step transitions) was field-tested and judged not to serve a real purpose — interrupts flow without giving the provider anything they need. Remove the component and its trigger sites, then re-evaluate the Narrative Thread chapter structure without the per-chapter intro overlay. Keep the data-model additions from PR 1 (intake `motivation_score`, `soft_commitment` fields) and the Prompter Sidebar from PR 2 — both still earn their place. Just drop the overlay and any plumbing that exists only to feed it.
