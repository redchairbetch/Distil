...
16. **Device Selection & Pricing Screen v1 — full spec build** — five-zone patient-facing screen (clinical context strip · recommendation + provider-editable rationale · within-family tier comparison · expandable detail panels for specs/what's-included/cross-mfr comparison/fit confirmation · purchase configuration + pricing). Recommendation engine inverts industry default — top tier is the starting point, the engine computes a down-tier justification score (0–4 / 5–8 / 9+) from audiometric + intake signals; rationale generated per-patient and provider-editable. Includes conditional "Available Rebates" panel (seasonal promo / mfr rebate / qualifying program), formal price-adjustment authorization modal with reason codes + manager-auth threshold + audit log, bundled/unbundled Complete Care+ purchase model with clinic-level default, payment options section with deferred-interest + equal-pay calculators (Allegro / CareCredit / HealthiPlan; Allegro down-payment shown as a separate line), provider adjustment-history reflection view, and three intake verbiage refinements (`hear`→`understand` in noisy-places, `sometimes`→`often` in speech-in-noise, split occupational vs recreational noise exposure). New schema: `device_catalog`, `cross_manufacturer_equivalence`, `rebate_promo`, `price_adjustment_log`, `purchase_configuration`, `purchase_line_item`, `clinic_settings` additions (`default_bundle_mode`, `override_manager_auth_threshold_percent`, `financing_partners`). Full spec lives at `OneDrive\Borderline Artistic\Distil\Distil.md` (v1.2, 2026-04-21). Multi-PR initiative — sequence per spec §12. Open dependencies: pricing-source-of-truth audit, intake→engine data pipeline, CC+ SKU structure, cross-mfr equivalence table population, financing partner URLs, adjustment-history view placement. v2 parks: provider-default-price, automated catalog sync, richer lifestyle modeling, discount analytics dashboards.
    ...
    **Status (2026-06-22 — audited vs shipped code):** Most of #16 already shipped incrementally. Done: schema (`purchase_configuration` / `purchase_line_item` / `price_adjustment_log` / `clinic_settings.default_bundle_mode`), device catalog population, the recommendation engine (`recommendationEngine.js` + `generateRecommendation`), Zones 1/2/3/5 in `views/DeviceSelection.jsx` (route `/distil/select/:id`), the §6 price-adjustment modal (`views/AdjustPriceModal.jsx` + `logPriceAdjustment`, PR #104 — manager-auth threshold deferred per closer-role decision), and the §9 occupational/recreational noise split. **Zone 4 detail panels — Device specifications · What's included · Audiogram fit-confirmation — shipped on branch `feat/device-screen-zone4-panels`:** collapsed-by-default accordions in DeviceSelection; `bundleMode` lifted from Zone5 to the parent so "What's included" reflects bundled/unbundled and the §7 unbundled state is visible; fit-confirmation renders only when `product_catalog_tier.fitting_range_*` is populated (semantics to confirm when Kurt fills them; dormant + hidden until then). Zone 4's environment-fit panel landed earlier (#25; shared `components/CoverageBars.jsx`). **Still open:** (a) **cross-manufacturer comparison (Zone 4c)** — needs the `cross_manufacturer_equivalence` table created + populated with tier-parity judgments across Signia/Phonak/Oticon/Starkey/ReSound/Widex (Kurt-owned per spec §11; no table exists yet); (b) **Available Rebates (§5)** — needs `rebate_promo` table + active promos (only the `purchase_line_item.rebate_promo_id` FK exists today); (c) **provider adjustment-history reflection view (§6/§11)** — `price_adjustment_log` is populated but never read back; placement TBD (settings vs patient-detail); (d) the **two remaining §9 intake verbiage tweaks** (`hear`→`understand` in noisy-places, `sometimes`→`often` in speech-in-noise) in `IntakeKiosk.jsx`. **Deferred (Kurt's call, 2026-06-22):** Payment Options / financing calculators (§8) = backlog #34.
